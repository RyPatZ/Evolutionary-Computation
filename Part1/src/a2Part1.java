...
//        System.out.printf("Enter train directory path");
        ...
        double[][][] delta_weights = nn.backward_propagate_error(instances[0], outputs[0], outputs[1], integer_encoded[0]);
...
//        System.out.printf("Enter test directory path");
        List<String[]> lines1 =  Util.getLines("penguins307-test.csv");
        String[] header1 = lines1.remove(0);
        String[] labels1 = Util.getLabels(lines1);
        double[][] instances1 = Util.getData(lines1);

        // scale features to [0,1] to improve training
        Rescaler rescaler1 = new Rescaler(instances1);
        rescaler1.rescaleData(instances1);

        // We can"t use strings as labels directly in the network, so need to do some transformations
        LabelEncoder label_encoder1 = new LabelEncoder(labels1);
        // encode "Adelie" as 1, "Chinstrap" as 2, "Gentoo" as 3
        int[] integer_encoded1 = label_encoder1.intEncode(labels1);

        int[] predictions=nn.predict(instances1);
        double acc = Double.NaN;
        int count = 0;
        int predictionCount=0;
        for (int de:integer_encoded1){
            if(de==predictions[predictionCount]){
                count++;
            }
            predictionCount++;
        }
        acc= (double) count/instances1.length*100;
        System.out.println("-------------------------------------------------------------------------------------------");
                    System.out.println("After "+100 +" of training\n We got "+ count + " out of " + instances1.length +" correct on test set");
